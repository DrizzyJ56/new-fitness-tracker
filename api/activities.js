...
const {getAllActivities, getPublicRoutinesByActivity, createActivity, updateActivity} = require("../db/index");
...
activitiesRouter.patch("/:id", requireUser, async (req,res,next) => {
    try {
        ...
        if(updateAct){
            ...
            next({
                name: "ActivityDoesNotExistError",
                message: "This activity id either does not exist, or an activity with this name exists"
            })
        }
    } catch ({name, message}) {
        next({name, message})
